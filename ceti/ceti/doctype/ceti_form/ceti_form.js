...
frappe.ui.form.on("CETI Form", {
	refresh: function(frm) {
		// show is-standard only if developer mode
		frm.get_field("is_standard").toggle(frappe.boot.developer_mode);

		frappe.ceti_form.set_fieldname_select(frm);
		...
		frm.add_custom_button(__('Get Fields'), () => {
			let webform_fieldtypes = frappe.meta.get_field('CETI Form Field', 'fieldtype').options.split('\n');
			...
			frappe.model.with_doctype(frm.doc.doc_type, () => {
				...
				for (let field of meta.fields) {
					if (webform_fieldtypes.includes(field.fieldtype)
						...
						frm.add_child('ceti_form_fields', {
...
frappe.ui.form.on("CETI Form Field", {
	...
	fieldname: function(frm, doctype, name) {
		...
		doc.fieldtype = frappe.meta.get_docfield("CETI Form Field", "fieldtype")
