...
class CETIForm(WebsiteGenerator):
	website = frappe._dict(
		no_cache = 1
	)

	def onload(self):
		super(CETIForm, self).onload()
		if self.is_standard and not frappe.conf.developer_mode:
			self.use_meta_fields()

	def validate(self):
		super(CETIForm, self).validate()
		...
		if (not (frappe.flags.in_install or frappe.flags.in_patch or frappe.flags.in_test or frappe.flags.in_fixtures)
			...
			frappe.throw(_("You need to be in developer mode to edit a Standard CETI Form"))
	...
	def validate_fields(self):
		...
		for df in self.ceti_form_fields:
...
""")
	...
	def get_context(self, context):
		'''Build context to render the `ceti_form.html` template'''
		self.set_ceti_form_module()
		...
		if frappe.form_dict.name and not has_ceti_form_permission(self.doc_type, frappe.form_dict.name):
	...
	def add_custom_context_and_script(self, context):
		...
		if self.ceti_form_module:
			new_context = self.ceti_form_module.get_context(context)

			if new_context:
				context.update(new_context)

			js_path = os.path.join(os.path.dirname(self.ceti_form_module.__file__), scrub(self.name) + '.js')
			...
			css_path = os.path.join(os.path.dirname(self.ceti_form_module.__file__), scrub(self.name) + '.css')
	...
	def set_ceti_form_module(self):
		'''Get custom web form module if exists'''
		self.ceti_form_module = self.get_ceti_form_module()

	def get_ceti_form_module(self):
		if self.is_standard:
	...
	def validate_mandatory(self, doc):
		...
		missing = []
		for f in self.ceti_form_fields:
...
def accept(ceti_form, data, docname=None, for_payment=False):
	'''Save the web form'''
	...
	ceti_form = frappe.get_doc("CETI Form", ceti_form)

	if data.name and not ceti_form.allow_edit:
		frappe.throw(_("You are not allowed to update this CETI Form Document"))

	frappe.flags.in_ceti_form = True
	...
	# set values
	for field in ceti_form.ceti_form_fields:
	...
	if for_payment:
		ceti_form.validate_mandatory(doc)
		doc.run_method('validate_payment')

	if doc.name:
		if has_ceti_form_permission(doc.doctype, doc.name, "write"):
		...
		# insert
		if ceti_form.login_required and frappe.session.user=="Guest":
	...
	frappe.flags.ceti_form_doc = doc

	if for_payment:
		return ceti_form.get_payment_gateway_url(doc)
	else:
		return doc

@frappe.whitelist()
def delete(ceti_form_name, docname):
	ceti_form = frappe.get_doc("CETI Form", ceti_form_name)

	owner = frappe.db.get_value(ceti_form.doc_type, docname, "owner")
	if frappe.session.user == owner and ceti_form.allow_delete:
		frappe.delete_doc(ceti_form.doc_type, docname, ignore_permissions=True)
	else:
		raise frappe.PermissionError("Not Allowed")


@frappe.whitelist()
def delete_multiple(ceti_form_name, docnames):
	ceti_form = frappe.get_doc("CETI Form", ceti_form_name)
...
def has_ceti_form_permission(doctype, name, ptype='read'):
...
@frappe.whitelist(allow_guest=True)
def get_ceti_form_filters(ceti_form_name):
	ceti_form = frappe.get_doc("CETI Form", ceti_form_name)
	return [field for field in ceti_form.ceti_form_fields if field.show_in_filter]
...
def get_form_data(doctype, docname=None, form_name=None):
	ceti_form = frappe.get_doc('CETI Form', form_name)

	if ceti_form.login_required and frappe.session.user == 'Guest':
		frappe.throw(_("Not Permitted"), frappe.PermissionError)

	out = frappe._dict()
	out.ceti_form = ceti_form

	if frappe.session.user != 'Guest' and not docname and not ceti_form.allow_multiple:
...
@frappe.whitelist(allow_guest=True)
def get_link_options(ceti_form_name, doctype, allow_read_on_all_link_options=False):
	ceti_form_doc = frappe.get_doc("CETI Form", ceti_form_name)
	doctype_validated = False
	limited_to_user   = False
	if ceti_form_doc.login_required:
		...
		for field in ceti_form_doc.ceti_form_fields:
